...
const PostPage = ({
  posts,
  frontmatter: { title, date, cover_image, read, excerpt, index },
  slug,
  content,
}) => {
  ...
  return (
    <Container>
      <Head>
        ...
        <meta
          content={config.navigation.colors.accent}
        ...
        <meta
          property="og:image"
          content={window.location.origin + cover_image}
        />
      ...
      <ItemContainer flex={width < 1100}>
        ...
        <FlexItem hide={!index} padding={width > 1100}>
          ...
          {randomPosts
            .filter((e) => e.frontmatter.index == true && e.slug != slug)
            .map((post, index) => {
              return (
                <MapItem
